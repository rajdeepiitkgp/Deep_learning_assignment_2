# -*- coding: utf-8 -*-
"""
Created on Sat Feb  9 11:38:10 2019
...
class MLP_DROP(nn.Block):
    def __init__(self,drop, **kwargs):
        super(MLP_DROP, self).__init__(**kwargs)
        self.blk = nn.Sequential()
        self.blk.add(nn.Dense(1024, activation='relu'),
                     nn.Dropout(drop),
                     nn.Dense(512, activation='relu'),
...
def vanilla_model_fit(no_epochs,batch_size,ctx,mx_train_data,mx_valid_data):
    train_loss_hist=[]
    train_acc_hist=[]
    valid_loss_hist=[]
    valid_acc_hist=[]
    ...
    mx_net.save_parameters('../weights/task_b_exp_3_vanilla.params')
...
def vanilla_model_test(batch_size,ctx,mx_test_data):
    mx_net = MLP()
    mx_net.load_parameters('../weights/task_b_exp_3_vanilla.params')
    mx_loss_fn = gluon.loss.SoftmaxCrossEntropyLoss()
...
def dropout_model_test(batch_size,ctx,mx_test_data,drop_prob):
    mx_net = MLP_DROP(drop_prob)
    mx_net.load_parameters('../weights/task_b_exp_3_dropout_'+str(int(10*drop_prob))+'.params')
    mx_loss_fn = gluon.loss.SoftmaxCrossEntropyLoss()
    test_acc,test_loss=0.0,0.0
    for data, label in mx_test_data:
        output=mx_net(data)
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        train_loss_hist_m1,train_acc_hist_m1,valid_loss_hist_m1,valid_acc_hist_m1 =vanilla_model_fit(no_epochs,batch_size,ctx,mx_train_data,mx_valid_data)
        print('Finished Traing the Model 1')
        
        print('\nTraing the Model 2 with Dropout = 0.1')
        train_loss_hist_m2,train_acc_hist_m2,valid_loss_hist_m2,valid_acc_hist_m2 =dropout_model_fit(no_epochs,batch_size,ctx,mx_train_data,mx_valid_data,drop_prob=0.1)
        print('Finished Traing the Model 2')
        
        print('\nTraing the Model 3 with Dropout = 0.4')
        train_loss_hist_m3,train_acc_hist_m3,valid_loss_hist_m3,valid_acc_hist_m3 =dropout_model_fit(no_epochs,batch_size,ctx,mx_train_data,mx_valid_data,drop_prob=0.4)
        print('Finished Traing the Model 3')
        
        print('\nTraing the Model 4 with Dropout = 0.6')
        train_loss_hist_m4,train_acc_hist_m4,valid_loss_hist_m4,valid_acc_hist_m4 =dropout_model_fit(no_epochs,batch_size,ctx,mx_train_data,mx_valid_data,drop_prob=0.6)
        ...
        labels = ["$\it{Vanilla}$ $\it{Network}$"]+l[:2]+["$\it{Dropout}$ $\it{=}$ $\it{0.1}$"]+l[2:4]+["$\it{Dropout}$ $\it{=}$ $\it{0.4}$"]+l[4:6]+["$\it{Dropout}$ $\it{=}$ $\it{0.6}$"]+l[6:]
        ...
        plt.savefig('task_b_exp_3_loss.png', format='png', dpi=600,bbox_inches='tight')
        plt.close('all')
        ...
        plt.savefig('task_b_exp_3_accuracy.png', format='png',dpi=600, bbox_inches='tight')
        plt.close('all')
    ...
    if args.test:
        ...
        if os.path.isfile('../weights/task_b_exp_3_vanilla.params'):
            vanilla_model_test(batch_size,ctx,mx_test_data)
        else:
            fileloader.download_file('task_b_exp_3_vanilla.params')
            vanilla_model_test(batch_size,ctx,mx_test_data)
            
        print('\nTesting the Model 2 with Dropout = 0.1')
        if os.path.isfile('../weights/task_b_exp_3_dropout_1.params'):
            dropout_model_test(batch_size,ctx,mx_test_data,drop_prob=0.1)
        else:
            fileloader.download_file('task_b_exp_3_dropout_1.params')
            dropout_model_test(batch_size,ctx,mx_test_data,drop_prob=0.1)
            
        print('\nTesting the Model 3 with Dropout = 0.4')
        if os.path.isfile('../weights/task_b_exp_3_dropout_4.params'):
            dropout_model_test(batch_size,ctx,mx_test_data,drop_prob=0.4)
        else:
            fileloader.download_file('task_b_exp_3_dropout_4.params')
            dropout_model_test(batch_size,ctx,mx_test_data,drop_prob=0.4)
            
        print('\nTesting the Model 4 with Dropout = 0.6')
        if os.path.isfile('../weights/task_b_exp_3_dropout_6.params'):
            dropout_model_test(batch_size,ctx,mx_test_data,drop_prob=0.6)
        else:
            fileloader.download_file('task_b_exp_3_dropout_6.params')
            dropout_model_test(batch_size,ctx,mx_test_data,drop_prob=0.6)
